...
class AirfoilNurbs(Airfoil):
    
    def __init__(self, initial_path='initial_airfoil/naca0012.dat', config_fname='op_conditions.ini'):
        ...
        # NACA 0012 as the initial airfoil
        self.alpha0, self.U_upper, self.U_lower = nurbs_airfoil(n_control_points, self.p, initial_path)
        self.dim = len(self.alpha0)
        self.bounds = np.zeros((self.dim, 2))
        perturb = 0.2
        self.bounds[:,0] = self.alpha0 - perturb
    ...
    def synthesize(self, alpha):
        alpha = np.array(alpha, ndmin=2)
        airfoils = np.apply_along_axis(lambda x: synthesize_nurbs(x, self.U_upper, self.U_lower, self.p, self.n_points), 1, alpha)
        return np.squeeze(airfoils)
    
    def fit(self, target_airfoil):
        
        def fun(x, target_airfoil):
            airfoil = synthesize_nurbs(x, self.U_upper, self.U_lower, self.p, self.n_points)
            error = np.mean(np.sum(np.square(airfoil-target_airfoil), axis=1))
            return error
        def jac(x, target_airfoil):
            fun_ = lambda x: fun(x, target_airfoil)
